...
public class MovieController {
    ...
    public String getDetailMovies(Model model, @PageableDefault(size = 5, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
        ...
        //비디오가 있는 인기순영화
        List<MovieDTO> videoMovies = movieService.getCachedMovies("videoMovies");
        model.addAttribute("videoMovies", videoMovies);

        //모든 영화에서 개봉일자가 4개월 전 부터 오늘날짜 까지인거만 가져옴 즉 현재 상영작임.
        List<MovieDTO> nowMovies = movieService.getCachedMovies("nowMovies");
        model.addAttribute("nowMovies", nowMovies);

        //모든 영화에서 개봉일자가 오늘날짜부터 4개월 후 까지인거만 가져옴 즉 상영예정작(개봉예정)임.
        List<MovieDTO> upcomingMovies = movieService.getCachedMovies("upcomingMovies");
        model.addAttribute("upcomingMovies", upcomingMovies);

        //인기순
        List<MovieDTO> hotMovies = movieService.getCachedMovies("hotMovies");
...
//        List<MovieDTO> movieInfos = movieService.getMovieDetailInfo(movieId);
        List<MovieDTO> movieInfos = movieService.getMovieFromRedis(movieId);
